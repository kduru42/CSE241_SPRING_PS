...
Organism* World::getAt(int x, int y){
    if((x>=0) && (x<WORLDSIZE) && (y>=0) && (y<WORLDSIZE)) return grid[x][y];
    return NULL;
}

void World::setAt(int x, int y, Organism* org){
    if((x>=0) && (x<WORLDSIZE) && (y>=0) && (y<WORLDSIZE)) grid[x][y] = org;
}
...
void Doodlebug::move(){
    ...
    cout << "a3";
    if((x>0) && (world->getAt(x-1, y)==NULL) && (world->getAt(x-1, y)->getType() == ANT)){
    ...
    cout << "a4";
    if((x<WORLDSIZE-1) && (world->getAt(x+1, y)==NULL) && (world->getAt(x+1, y)->getType() == ANT)){
        delete (world->grid[x+1][y]);
        world->grid[x+1][y] = this;
        world->setAt(x, y, NULL);
        starveTicks = 0;
